# el34 Assessor — 읽기 전용 평가 수집 레이어

중앙 플랫폼(CC/tubewar)이 학생별 el34 VM에서 **채점·모니터링에 필요한 상황정보를
읽기 전용으로 당겨갈 수 있도록** 추가된 별도 서비스다. Bastion·토폴로지·취약웹·Wazuh
코어는 일절 손대지 않는다.

> **클라이언트는 문맥 없이(dumb) 수집만 한다.** 과목/학년/반/팀(Cohort)·index 분리
> 로직은 el34 안에 없다 — 그건 전적으로 서버(tubewar)의 책임이다. 같은 el34 이미지가
...
| 항목 | 값 |
|------|----|
| 컨테이너 | `el34-assessor` (dmz, `10.20.32.55`) |
| 스택 | `python:3.12-slim` + FastAPI + docker SDK + httpx |
| 외부 노출 | `http://assessor.el34.lab/` (fw 경유, WAF 우회 — portal 과 동일) |
...
| 토글 | `SKIP_ASSESSOR=1 bash el34.sh up` → 생성 안 함(base 무영향) |
| (옵션) provisioner | `el34-provisioner` (dmz `10.20.32.56`) — write 서비스, **기본 OFF**(`SKIP_PROVISIONER=0` 으로만 기동). §10 |

Assessor 는 compose profile `assessor` 로 묶여 있어 `bash el34.sh up` 이 기본 활성화하고,
...
### `GET /health` (인증 불필요)
```json
{ "status": "ok", "service": "el34-assessor",
...
```json
{
  ...
  "battle_id": "optional",
  "results": [
    { "id": "c1", "passed": true, "evidence": "12:SecRuleEngine On", "raw": { "exit_code": 0, "container": "el34-web" } }
  ]
}
```
...
`target` 또는 `container` 에 아래 표준 별칭(또는 `el34-` 컨테이너명)을 쓴다. 어느 컨테이너에
exec 할지는 Assessor 내부 맵이 결정한다 — **클라이언트는 토폴로지를 몰라도 된다.**

| 별칭 | 컨테이너 | 비고 |
|------|----------|------|
| `fw` (=`firewall`,`secu`) | el34-fw | nftables (L3 DNAT) |
| `ips` (=`ids`,`suricata`) | el34-ips | Suricata |
| `web` (=`waf`,`apache`) | el34-web | Apache + ModSecurity |
| `siem` (=`wazuh`,`manager`) | el34-siem | Wazuh manager |
...
ASSESS="curl -s -H Host:assessor.el34.lab -H X-API-Key:$KEY -H Content-Type:application/json -X POST http://<VM_IP>/assess -d"
...
ACT="curl -s -H Host:assessor.el34.lab -H X-API-Key:$KEY -H Content-Type:application/json -X POST http://<VM_IP>/activity -d"
...
`fim_change`/`command_ran` 질의가 가능하도록 el34 의 Wazuh 수집을 **모든 학생 동일하게
...
- **FIM (syscheck, realtime + report_changes + whodata)** — `web`(apache/modsec 설정 + /home/ccc),
  ...
  각 컨테이너 `entrypoint.sh` 가 `<ossec_config>` 에 `el34-assessor-collection` 블록을 1회 주입(멱등).
- **명령 로깅** — 모든 대화형 셸이 `/etc/profile.d/el34-cmdlog.sh` 의 `PROMPT_COMMAND` 로
  `CMDEL34 host=… user=… pwd=… rc=… cmd=…` 라인을 남긴다.
  - `attacker`/`bastion`(Wazuh agent 없음): 기존 `rsyslog *.* @siem:514` 로 manager 전달.
  - `web`/`fw`/`ips`(Wazuh agent 보유): `/var/log/el34-cmd.log` localfile 로 manager 전달.
...
## 9. cross-infra 듀얼 — VM ↔ VM 도달성 (el34 측 점검)

tubewar 가 "학생 A 가 학생 B 의 VM 을 공격"하는 cross-infra 듀얼을 운영한다. el34 측 결론:
...
- A 의 `attacker` → B 의 `VM_IP:80`(`Host: juice.el34.lab` 등) → **B 의 fw → ips(Suricata) →
...
- B 의 `int`(취약웹 7종, 10.20.40.0/24)은 **직접 노출 없음** — B 의 web vhost reverse proxy 로만
  도달(호스트에서 `docker port el34-juiceshop` 없음으로 확인). 따라서 A 는 B 의 내부망에 직접 못 들어간다.
- Assessor(10.20.32.55)는 dmz 내부 서비스로 **별도 published 포트가 없다.** CC 는 `VM_IP:80` 에
  `Host: assessor.el34.lab` + `X-API-Key` 로만 접근(공격자에겐 의미 없는 읽기 전용 API).
...
| 항목 | 값 |
|------|----|
| 컨테이너 | `el34-provisioner` (dmz `10.20.32.56`, profile `provisioner`) |
| 기동 | **기본 OFF.** `SKIP_PROVISIONER=0 bash el34.sh up` 으로만. (외부: `provisioner.el34.lab`) |
| 인증 | `X-API-Key` |
| 안전장치 | named 템플릿 화이트리스트만 · sid 슬롯 `110000–119999` 자동 할당 · 전용 파일 `zz-el34-provisioned-rules.xml` 하나만 write(다른 룰/디코더 불변) · **반영 전 `wazuh-analysisd -t` 검증 → 실패 시 롤백**(잘못된 룰이 manager 를 못 깨뜨림) |
...
```bash
KEY=ccc-api-key-2026
PROV="curl -s -H Host:provisioner.el34.lab -H X-API-Key:$KEY -H Content-Type:application/json -X POST http://<VM_IP>"
