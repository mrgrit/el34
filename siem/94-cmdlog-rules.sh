#!/bin/sh
# cont-init: el34 명령 로깅 decoder + rules 를 wazuh manager 에 강제 적용.
...
if [ -f "$IMG_DEC" ] && [ -f "$IMG_RULES" ]; then
  ...
  echo "[siem] ★ el34 cmdlog decoder + rules applied"
fi
# 명령 로그(attacker/bastion)는 기존 syslog remote :514 (96-bastion-audit-rules 가 활성)로 수신.
# web/fw/ips 는 Wazuh agent localfile(/var/log/el34-cmd.log)로 수신.
