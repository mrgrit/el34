...
def build_planning_prompt(vm_ips: dict[str, str] = None,
    ...
    sections.append(
        ...
        "## ★ 6v6 컨테이너 인프라 컨텍스트 (반드시 준수)\n\n"
        "너는 **6v6-bastion 컨테이너 내부**에서 실행되고 있다. 학습용 lab instruction\n"
        "은 '학생 PC 에서 ssh 6v6-bastion <cmd>' 처럼 학생 관점으로 작성되어 있을 수\n"
        "있다. 이 패턴은 너에게는 의미가 없다. 다음 규칙으로 **반드시** 변환하라:\n\n"
        "  (R1) `ssh 6v6-bastion '<CMD>'` 또는 `ssh 6v6-bastion <CMD>` →\n"
        "        target_vm=manager (=너 자신), script=<CMD> 만. ssh 래퍼 제거.\n"
        "        manager 는 로컬 실행이므로 ssh 인증 없이 즉시 동작.\n\n"
        "  (R2) `ssh 6v6-<X> '<CMD>'` (X = fw / ips / web / siem / attacker 등) →\n"
        "        target_vm 을 다음 매핑으로 선택, script=<CMD> 만:\n"
        "          6v6-fw   → secu       6v6-ips  → secu\n"
        "          6v6-web  → web        6v6-attacker → attacker\n"
        "          6v6-siem → siem       6v6-wazuh-*  → siem\n"
        "          6v6-juiceshop / dvwa / neobank / govportal / mediforum / portal / \n"
        "          adminconsole / aicompanion → web (모두 web 호스트의 docker 컨테이너)\n\n"
        "  (R3) `docker exec 6v6-<X> <CMD>` 또는 `docker logs 6v6-<X>` 처럼 docker\n"
        ...
        "  (R5) 절대 금지: `ssh 6v6-bastion` 명령을 그대로 shell skill 에 전달하지 마라.\n"
        ...
        "        학습 환경 (10.20.30.x / 10.20.31.x / 10.20.32.x / 10.20.40.x / juice.6v6.lab\n"
        "        / dvwa.6v6.lab / siem.6v6.lab / 192.168.0.109:11434 / 192.168.0.110:9200)\n"
