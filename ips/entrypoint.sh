...
if [ -n "$DMZ_IFACE" ]; then
    nft "add table ip el34mgr" 2>/dev/null || true
    nft "add chain ip el34mgr postrouting { type nat hook postrouting priority 90 ; }" 2>/dev/null || true
    nft "add rule ip el34mgr postrouting oifname \"$DMZ_IFACE\" ip daddr $WAZUH_MANAGER ip saddr 10.20.30.0/24 masquerade" 2>/dev/null || true
    nft "add rule ip el34mgr postrouting oifname \"$DMZ_IFACE\" ip daddr $WAZUH_MANAGER ip saddr 10.20.31.0/24 masquerade" 2>/dev/null || true
...
if [ "${PRESERVE_SRC_IP:-1}" = "1" ]; then
    ...
    nft "add table ip natel34" 2>/dev/null || true
    nft "add chain ip natel34 postrouting { type nat hook postrouting priority 100 ; }" 2>/dev/null || true
    nft "add rule ip natel34 postrouting oifname \"$DMZ_IFACE\" ip saddr 10.20.30.0/24 masquerade" 2>/dev/null || true
    nft "add rule ip natel34 postrouting oifname \"$DMZ_IFACE\" ip saddr 10.20.31.0/24 masquerade" 2>/dev/null || true
fi
...
if [ -d /var/ossec ]; then
  ...
  <localfile>\n    <log_format>json</log_format>\n    <location>/var/log/suricata/eve.json</location>\n  </localfile>\n  <localfile>\n    <log_format>syslog</log_format>\n    <location>/var/log/syslog</location>\n  </localfile>' /var/ossec/etc/ossec.conf
    ...
    # el34-assessor: FIM(suricata 설정/룰 + 실습 디렉터리) + 명령 로깅 localfile (정적·cohort-free, 멱등)
...
# ips-gui.el34.lab 가 네트워크/exec 없이 즉시 열린다(HAProxy 라우트는 base config 내장).
