#!/usr/bin/env bash
# secuops-easy 특강 배포/복구 — el34 호스트에서 실행 (★오프라인·멱등★).
#
# 2026-06 구조 변경:
...
#
#   대상 컨테이너: el34-fw / el34-ips / el34-web  (port 8080)
#   접속(학생): http://fw-gui.el34.lab  /  http://ips-gui.el34.lab  /  http://waf-gui.el34.lab
set -uo pipefail
...
GUIS=(
  "nft_edu_gui:el34-fw:fw-gui"
  "suricata_edu_gui:el34-ips:ips-gui"
  "modsec_edu_gui:el34-web:waf-gui"
)

echo "== [1/4] WAF(ModSecurity) 설정 보정(멱등) + Apache 확인 =="
if docker ps --format '{{.Names}}' | grep -q '^el34-web$'; then
  docker cp "$HERE/fix_modsec.py" el34-web:/tmp/fix_modsec.py 2>/dev/null && \
    docker exec el34-web python3 /tmp/fix_modsec.py 2>/dev/null || true
  # Apache 는 entrypoint 가 기동. 혹시 죽어 있으면 best-effort 재기동.
  if ! docker exec el34-web pgrep -x apache2 >/dev/null 2>&1; then
    docker exec el34-web bash -c 'apache2ctl configtest && service apache2 start' 2>/dev/null || true
  fi
fi

echo "== [2/4] Suricata 룰 baseline 보정(멱등) + reload =="
if docker ps --format '{{.Names}}' | grep -q '^el34-ips$' && \
   docker exec el34-ips test -d /etc/suricata/rules 2>/dev/null; then
  docker cp "$HERE/suricata_local.rules.baseline" el34-ips:/etc/suricata/rules/local.rules 2>/dev/null || true
  docker exec el34-ips suricatasc -c reload-rules 2>/dev/null || true
fi
...
for spec in "${GUIS[@]}"; do
  ...
  if [ -f "$GUI_SRC/$repo/server.py" ]; then
    ...
    echo "  $vhost: ! vendored 소스 없음 ($GUI_SRC/$repo) — 'bash el34.sh up' 로 이미지 재빌드 필요"
  fi
done

echo "== [4/4] HAProxy GUI 라우트 확인 (base config 내장; 누락 시에만 backward-compat patch) =="
if docker ps --format '{{.Names}}' | grep -q '^el34-fw$'; then
  if docker exec el34-fw grep -q 'is_fw_gui' /etc/haproxy/haproxy.cfg 2>/dev/null; then
    echo "  HAProxy: GUI 라우트가 base config 에 존재 → patch 불필요"
  else
    echo "  HAProxy: GUI 라우트 누락(구 이미지) — patch 적용 후 안전 reload"
    docker exec el34-fw cp /etc/haproxy/haproxy.cfg "/etc/haproxy/haproxy.cfg.bak" 2>/dev/null || true
    docker cp "$HERE/patch_haproxy.py" el34-fw:/tmp/patch_haproxy.py
    if docker exec el34-fw python3 /tmp/patch_haproxy.py | grep -q PATCHED; then
      if docker exec el34-fw haproxy -c -f /etc/haproxy/haproxy.cfg >/dev/null 2>&1; then
        docker exec el34-fw bash -c 'haproxy -f /etc/haproxy/haproxy.cfg -sf $(pgrep -o haproxy) -D' \
...
for spec in "${GUIS[@]}"; do
  vhost="${spec##*:}"
  docker ps --format '{{.Names}}' | grep -q '^el34-fw$' || { echo "  $vhost: SKIP (el34-fw down)"; continue; }
  title=$(docker exec el34-fw curl -s -m 5 -H "Host: $vhost.el34.lab" "http://127.0.0.1/" 2>/dev/null \
            | grep -ioE '<title>[^<]*</title>' | head -1)
  case "$title" in
    *콘솔*) echo "  [OK]   $vhost.el34.lab => $title" ;;
    *)      echo "  [FAIL] $vhost.el34.lab => '${title:-no-response}' (콘솔 아님 — 'bash el34.sh up' 재빌드 필요)" ;;
  esac
done
echo "done. 학생 접속: http://{fw,ips,waf}-gui.el34.lab"
