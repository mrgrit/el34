...
보안시스템 입문 특강(secuops-easy, 6주)에서 쓰는 세 개의 장비형 교육용 GUI 를 el34 인프라에
...
|------|----------|-----------|-----------|
| 방화벽(nftables) | `mrgrit/nft_edu_gui` | el34-fw :8080 | http://fw-gui.el34.lab |
| IPS(Suricata) | `mrgrit/suricata_edu_gui` | el34-ips :8080 | http://ips-gui.el34.lab |
| WAF(ModSecurity) | `mrgrit/modsec_edu_gui` | el34-web :8080 | http://waf-gui.el34.lab |
...
배포 과정에서 다음 보정을 함께 적용한다(2026-05-27 el34 실측 기준 필요했던 수정).
...
보통은 **아무것도 할 필요 없다** — `bash el34.sh up` 이 이미지 빌드 시 GUI 를 내장하고,
...
```bash
bash el34.sh smoke   # "교육용 콘솔" 섹션이 세 콘솔의 실제 title 을 확인 (랜딩 fallthrough 거짓 200 차단)

# 수동 확인 — 200 만 보면 안 되고 title 에 '콘솔' 이 있어야 진짜 콘솔이다:
for g in fw ips waf; do
  curl -s -H "Host: $g-gui.el34.lab" http://<VM_IP>/ | grep -o '<title>[^<]*</title>'
done
docker exec el34-ips suricatasc -c ruleset-stats   # rules_loaded>=5, failed 0
docker exec el34-web apache2ctl configtest          # Syntax OK
```
...
## 강의 콘텐츠
교안/실습(6주)은 CCC 레포 `contents/standalone/{lecture,lab}/secuops-easy/` 에 있으며 el34 Training
